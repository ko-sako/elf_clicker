...
pub async fn in_game_hard_update(internal_state: &mut InternalState) -> GameState {
    play_sound(
        &internal_state.ingame_hard_bgm,
        PlaySoundParams {
            looped: true,
            volume: 0.2,
        },
    ...
    loop {
        ...
        if internal_state.clicks < 5 {
            draw_texture(
                ...
                internal_state.elf.x
                    - &internal_state.frames[internal_state.frame_index].width() / 2.0,
        ...
        // elf2
        if internal_state.clicks2 < 5 {
            draw_texture(
                &internal_state.frames2[internal_state.frame_index],
                internal_state.elf2.x
                    - &internal_state.frames2[internal_state.frame_index].width() / 2.0,
                internal_state.elf2.y
                    - &internal_state.frames2[internal_state.frame_index].height() / 2.0,
                WHITE,
            );
        }
        ...
        if is_mouse_button_pressed(MouseButton::Left) {
            ...
            let distance_from_elf2 = vec2(mouse_x, mouse_y).distance(internal_state.elf2);
            ...
            } else if distance_from_elf2 <= elf_r + 8.0 {
                // hit
                internal_state.clicks2 += 1;
                play_sound(
                    &internal_state.gun_sound,
                    PlaySoundParams {
                        looped: false,
            ...
            internal_state.msg = format!(
                ...
                internal_state.clicks + internal_state.clicks2,
            ...
            internal_state.elf2.x = gen_range(30.0, screen_width() - 30.0);
            internal_state.elf2.y = gen_range(50.0, shelf_y - elf_r - 10.0);
        }

        draw_text(&internal_state.msg, 20.0, 30.0, 28.0, WHITE);

        if internal_state.clicks == 5 && internal_state.clicks2 == 5 {
            stop_sound(&internal_state.ingame_hard_bgm);
